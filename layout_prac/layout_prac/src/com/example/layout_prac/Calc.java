	...
	public static String calc(String textexpression) {
		...
		for(int i=0; i<arr_postExp.size(); i++) {
			if ( !arr_postExp.get(i).equals("+") && !arr_postExp.get(i).equals("-") && !arr_postExp.get(i).equals("x") && !arr_postExp.get(i).equals("÷")
					 ) { //|| arr_postExp.get(0).toString().charAt(0) == '-'
				...
				case 'x': 
					...
					break;
				case '÷': 
					result = bd_num2.divide(bd_num1, MathContext.DECIMAL32); //32는7글자로 디파인 되있음.
					stack_num.push(result);
					break;
				}
			}
		...
		if (tmpResult.charAt(tmpResult.length()-1) == '0' && tmpResult.charAt(tmpResult.length()-2) == '.') 
			result = new BigDecimal(tmpResult.substring(0, tmpResult.length()-2));
		//.0으로 끝나는 경우 .0을 지워서 result에 삽입
		...
		if (textexpression.charAt(0)=='0') { //0으로시작(-로시작)하고 이게 내가친0이아니고 ans값에서 나온거라면 0짜르고 보여주게
			String tmpexp = textexpression.substring(1, textexpression.length());
			return tmpexp.concat("=" + MainActivity.arr_sum.get(0));
		}
		else
